...
def open_file(dataset):
    _, ext = os.path.splitext(dataset)  # 分离文件名与扩展名
    ext = ext.lower()  # 字符串中所有大写字符为小写
    if ext == '.mat':
        # Load Matlab array
        return io.loadmat(dataset)
    elif ext == '.tif' or ext == '.tiff':
        # Load TIFF file
        return imageio.imread(dataset) # misc.imread(dataset)
    elif ext == '.hdr':
...
def convert_from_color_(arr_3d, palette=None):
    """Convert an RGB-encoded image to grayscale labels. 这里与convert_to_color_()作用相反
...
def display_predictions(pred, vis, gt=None, caption="222"):
    if gt is None:
...
def explore_spectrums(img, complete_gt, class_names, vis,
    ...
    mean_spectrums = {}
    std_spectrums_ = {}
    mean_spectrums_mat = np.ones((16, 204))
    for c in np.unique(complete_gt):
        ...
        for spectrum in class_spectrums[::step, :]:
            plt.plot(spectrum, alpha=0.25)      # 绘出每个类别的波段数据
        ...
        plt.fill_between(range(len(mean_spectrum)), lower_spectrum,   # 在函数lower_spectrum和higher_spectrum之间区域填充颜色
                         higher_spectrum, color="#3F5D7D")
        plt.plot(mean_spectrum, alpha=1, color="#FFFFFF", lw=2)
        vis.matplot(plt)
        mean_spectrums[class_names[c]] = mean_spectrum
        mean_spectrums_mat[c-1] = mean_spectrum
        std_spectrums = np.std(mean_spectrums_mat, axis=0)
        # std_spectrums_[class_names[c]] = std_spectrums[c-1]
    return mean_spectrums, std_spectrums
...
def plot_spectrums_(spectrums, vis, title=""):
    """Plot the specified dictionary of spectrums.
    ...
    #     # n_bands = len(v)
    #     update = None if win is None else 'append'
    #     win = vis.line(X=np.arange(1), Y=spectrums, name=k, win=win, update=update,
    #                    opts={'title': title})
    win = vis.line(X=np.arange(204), Y=spectrums, name=1, win=None, update=None,
                   opts={'title': title})
...
def sliding_window(image, step=10, window_size=(20, 20), with_data=True):     # step 从10改为了1
...
def count_sliding_window(top, step=10, window_size=(20, 20)):    # step 从10改为了1
...
def metrics(prediction, target, ignored_labels=[], n_classes=None):
    ...
    prediction = prediction[ignored_mask]   # 这一波操作是将Undefined类别从结果中移除，不进行测评
    ...
    list1 = []
    # list_diag = np.diag(cm)  # 获取confusion_matrix的主对角线所有数值
    for i in range(1, len(cm)):
        try:
            list1.append(cm[i, i]/np.sum(cm[i, :]))  # 单个类别的准确率
        except ZeroDivisionError:
            cm[i] = 0.
            list1.append(cm[i])
    # list1 obtain all class accuracy
    AA = np.mean(list1)
    results["Average_accuracy"] = AA
...
def show_results(results, vis, label_values=None, agregated=False):
    ...
    if agregated:
        ...
        Average_accuracy = [r["Average_accuracy"] for r in results]
    ...
    if agregated:
        text += ("Overall_Accuracy: {:.04f} +- {:.04f}\n".format(np.mean(accuracies),
                                                         np.std(accuracies)))
    else:
        text += "Overall_Accuracy : {:.04f}%\n".format(accuracy)
    text += "---\n"

    if agregated:
        text += ("Average_Accuracy: {:.04f} +- {:.04f}\n".format(np.mean(Average_accuracy),
                                                      np.std(Average_accuracy)))
    else:
        text += "Average_Accuracy: {:.04f}\n".format(Average_accuracy)
    text += "---\n"
...
def sample_gt(gt, train_size, mode='random'):  # 输入gt_，无扩展的图片
    ...
    y = gt[indices].ravel()  # classes   # ravel()将多维数组转换为一维数组的功能,y只是收集有标签的类别
    train_gt = np.zeros_like(gt)  # gt全零化
    ...
    if mode == 'random':
        ...
        ' train_test_split用于将矩阵随机划分为训练子集和测试子集，并返回划分好的训练集测试集样本和训练集测试集标签。 '
...
def camel_to_snake(name):
    s = re.sub('(.)([A-Z][a-z]+)', r'\1_\2', name)  # 替换的一个函数
...
def get_train(gt, num, N_CLASSES):
    ...
    index = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []]  # paviau:10 indean:17，这里记住要做更改
    index2 = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []]
    for x, y in indice:
        for i in range(1, N_CLASSES):
            # if x > p and x < gt.shape[0] - p and y > p and y < gt.shape[1] - p:
            if gt[x][y] == i:
                pos = (x, y)
                index[i].append(pos)

    for i in range(1, N_CLASSES):
        index2[i] = random.choices(index[i], k=num)
        index[i] = list(set(index[i]) - set(index2[i]))  # 保存剔除掉index2的元素即test的元素
        for x, y in index2[i]:
            mask[x, y] = gt[x, y]
        for x, y in index[i]:  # test_gt
            mask2[x, y] = gt[x, y]
    return mask
